...
class OrderImportController(http.Controller):
    @http.route('/api/v1/order/import', type='json', auth='api_key', methods=['POST'], csrf=False)
    ...
    def _get_or_create_product(self, line_data):
        ...
        # Vérification par référence interne (identifiant unique)
    ...
    def _get_or_create_trainer(self, trainer_name):
        partner_obj = request.env['res.partner']
        # Vérification par nom (identifiant unique pour les formateurs)
